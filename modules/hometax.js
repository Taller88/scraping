    ...
    this.header['Cookie'] = this.cookies;
       ...
       console.log("test postData:"+this.postData);
        ...
        if(result.data.resultCode === '401'){
            var error = {};
            error['resultCode']= result.data.resultCode;
            error['resultMsg']=result.data.clientMessage;
            return error;
        }
        
        var success = {}
        success.data['userName'] = userName;
        success.data['userPhone'] = userPhone;
        success.data['birth'] = birth;
        ...
        if (err.response) {
            ...
            var error = {};
            console.log(err.response.data.clientMessage);
            console.log(err.response.status);
            console.log(err.response.headers);
            error['resultCode']= err.response.status;
            error['resultMsg']=err.response.data.clientMessage
          }
        return error
        // next(err)// next(없음)
    }
